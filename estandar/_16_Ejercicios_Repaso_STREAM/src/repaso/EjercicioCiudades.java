...
public class EjercicioCiudades {
	...
	// devolver el CONJUNTO de Ciudades cuya TEMPERATURA sea MENOS que la pasada por parámetro
	public Set<Ciudad> ciudades(List<Ciudad> lst, double temp){
		
		return lst.stream()
				.filter(c -> c.getTemperatura()<temp) // Stream<Ciudad>
				.collect(Collectors.toSet()); // Set<Ciudad> (Conjunto de Ciudades)
